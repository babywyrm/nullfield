// Package mcp holds the JSON-RPC 2.0 envelope that MCP rides on.
//
// It lives apart from pkg/proxy because both inbound adapters need to read a
// tools/call: the HTTP proxy and the ext_authz decision service. Having the
// latter import the former would make one front door depend on another and drag
// policy, identity, audit and scope along transitively for the sake of two
// structs and a parse.
package mcp
