...
- **The quickstart pointed at demos instead of showing anything.** Its steps read "Follow Demo 04", and three of them said "if available" about demos that have no test script and may not run. Someone evaluating nullfield had to pick a demo, run it, and infer the product from it. It is now self-contained: bring up compose, watch a call succeed, watch two different refusals, read what was recorded, and change policy on the running proxy. Every command and response in it was executed against the checked-in demo configuration rather than written from memory, which is how the wrong `reason_class` (`denied_by_rule`, actually `policy_denied`) was caught before shipping.

  It also states the dev-mode fail-open plainly rather than leaving it implicit. With no identity provider configured the noop verifier fabricates a `dev-user` for every request, so `requireIdentity: true` in the demo policy is satisfied by a caller presenting nothing at all. That is fine locally and is a gate that reviews as configured and enforces nothing in a cluster, so the production checklist now pairs "configure a JWKS provider" with "verify an unauthenticated call is actually refused" as one item.

- **`examples/tools.yaml` gains `github_delete_repo`**, registered and deliberately in no ALLOW rule. Every refusal the demo could previously produce came from the registry, so the policy engine was never observed saying no and `-32000` appeared in no example, test, or document. A known tool that nobody is authorized to call is what makes the two gates distinguishable — the registry answers whether a tool exists, policy answers whether you may call it, and telling those apart is what separates a client drifting out of sync from something probing for authority it does not have.

- **`docs/architecture.md` no longer implies the tool registry hot-reloads.** It described `pkg/registry` as "thread-safe for hot-reload", which is true about the type and misleading about the system: policy is polled every ten seconds and the registry is not watched at all, so a `tools.yaml` edit does nothing until restart. Both the asymmetry and the reason for it are now stated where each is relevant.
...
- **The smoke test's last assertion had never been achievable.** It sent `GET /` and expected HTTP 200 to prove non-JSON-RPC traffic is passed through untouched. A GET carries no body, so the echo server cannot parse it as JSON-RPC and answers 400 — its own reply, which is *itself* the proof that nullfield forwarded rather than judged. The assertion was reading the upstream's opinion of an empty body and reporting it as a proxy failure, so the suite the README now points newcomers at opened with a red line for a reason unrelated to nullfield. Now asserted on the response body.

  The same suite gained the policy-denial case it never had: fourteen assertions, and until now a rule engine that denied nothing would have passed all of them, because every refusal it checked stopped at the registry.
...
- **Enforce mode no longer reports a counterfactual on a decision it applied.** `docs/observability.md` has always said the field is "present when the decision was **not** applied", and that one on an applied decision "would be a bug", but the emission was unconditional at both sites — the policy path and the truncated-body refusal. Caught on a live cluster: a call that genuinely received a 403 also logged `counterfactual: DENY`. The cost is interpretive rather than structural. An operator sizing up a rollout filters on this field to ask which calls a shadow deployment *would* have blocked, and enforce-mode events answering that query inflate the count with denials that already happened, implying shadowing that never occurred. Observe and no-op are unchanged. Now covered by tests for both sites and all three modes, which had only ever exercised observe.
